...
import com.huawei.l00379880.exam.qo.ForgetQo;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public User forgetPassword(HttpServletRequest request) {
        User user;
        String loginType = (String) request.getAttribute("loginType");
        String userInfo = (String) request.getAttribute("userInfo");
        if (LoginTypeEnum.USERNAME.getType().equals(loginType)) {
            // 登陆者用地是用户名
            user = userRepository.findByUserUsername(userInfo);
        } else {
            // 登陆者用地是邮箱
            user = userRepository.findByUserEmail(userInfo);
        }
        System.out.println("好的!");
        if (user != null) {
            System.out.println(user.getValiQuestion());
            System.out.println(user.getValiQuestionAnswer());
        }
        return null;
    }
